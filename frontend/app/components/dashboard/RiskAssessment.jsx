...
import useProfessionData, { getRiskLevel, getTopTasks } from "../../hooks/useProfessionData";
...
export default function RiskAssessment() {
    ...
    useEffect(() => {
        ...
        const riskPercentage = Math.round(professionData.weighted_final_score * 100);
        ...
        const highScenario = professionData.future_scenarios?.find(s => s.likelihood === "high");
            ...
            <div className="relative">
                    ...
                    <div className="relative w-48 h-48">
                        <svg className="w-full h-full transform -rotate-90">
                            ...
                            <motion.circle
                                ...
                                r="88"
                                stroke={
                                    riskData.riskLevel === "high" ? "#ef4444" :
                ...
                <div className="bg-white/5 rounded-lg p-4 mb-6">
                    <p className="text-sm text-zinc-300 leading-relaxed">
                        Based on current AI capabilities, this role has a <strong className={
                            riskData.riskLevel === "high" ? "text-red-400" :
                                riskData.riskLevel === "medium" ? "text-yellow-400" :
                                    "text-green-400"
                        }>{riskData.riskLevel} automation risk</strong> with significant changes expected within <strong className="text-white">{riskData.timeframe}</strong>.
                    </p>
                </div>
                        ...
                        >
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-zinc-500">{item.timeShare}% of time</span>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6">
                    <div className="bg-white/5 rounded-lg p-3 text-center">
                        ...
                        <p className="text-sm font-semibold text-white capitalize">{riskData.riskLevel}</p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 text-center">
                        <TrendingUp className="w-5 h-5 text-blue-400 mx-auto mb-1" />
                        <p className="text-xs text-zinc-400">Trend</p>
                        <p className="text-sm font-semibold text-white">
                            {riskData.riskLevel === "high" ? "Declining" : riskData.riskLevel === "medium" ? "Stable" : "Growing"}
                        </p>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 text-center">
                        <Clock className="w-5 h-5 text-green-400 mx-auto mb-1" />
                        <p className="text-xs text-zinc-400">Timeframe</p>
                        <p className="text-sm font-semibold text-white">{riskData.timeframe}</p>
                    </div>
                </div>
            </div>
        </motion.div>
