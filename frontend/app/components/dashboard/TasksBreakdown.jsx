...
export default function TasksBreakdown() {
    ...
    useEffect(() => {
        ...
        const taskList = Object.entries(professionData.task_automation_breakdown).map(([taskName, taskData]) => {
            ...
            return {
                name: shortName.length > 150 ? shortName.substring(0, 150) + "..." : shortName,
                fullName,
    ...
    if (error || tasks.length === 0) {
        return (
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
                <div className="flex items-center justify-center h-64">
                    <div className="text-zinc-400">{error || "No tasks data available"}</div>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
            <div className="mb-6">
                <h2 className="text-2xl font-bold text-white mb-2">Task Automation Analysis</h2>
                    ...
                    >
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 text-xs text-zinc-400">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span>{task.timeShare}% of time</span>
                                    </div>
                                    <div className={`flex items-center gap-1 ${task.color} font-semibold`}>
                                        <AlertCircle className="w-3 h-3" />
                                        <span>{task.automation}% automation risk</span>
                                    </div>
                                </div>
                            </div>
                            <div className={`px-3 py-1 rounded-full text-xs font-semibold ${task.bgColor} ${task.color} border border-current`}>
                        ...
                        <div className="space-y-2">
                            ...
                            <div>
                                ...
                                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                                    <div
                                        className={`h-full ${task.riskLevel === "high" ? "bg-red-500" :
                                            task.riskLevel === "medium" ? "bg-yellow-500" :
                                                "bg-green-500"
                                            }`}
