...
export const InputNode = ({ data }) => (
  ...
  >
    <img
      src={formImage}
      alt="User Profile Form"
      className="w-full h-auto border border-white/20 rounded-2xl"
    />
  </motion.div>
);
...
export const CompetitionNode = () => (
  ...
  >
    ...
    <div className="max-w-4xl relative">
      {/* Decorative quotes */}
      <span className="absolute -top-8 -left-12 text-8xl text-blue-500/20 font-serif">
        "
      </span>
      ...
      <span className="absolute -bottom-12 -right-12 text-8xl text-blue-500/20 font-serif rotate-180">
        "
      </span>
    </div>
  </motion.div>
...
export const DecompositionNode = ({ title, items, type = "tasks" }) => (
  <NodeContainer className="w-[500px] border-neon-cyan/30" delay={0.2}>
    <div className="flex items-center gap-4 mb-4">
      <div className="p-3 rounded-full bg-neon-cyan/10 text-neon-cyan">
        <Activity size={40} />
      </div>
      <h3 className="font-mono text-2xl font-bold text-neon-cyan">
        {title.toUpperCase()}
      </h3>
    </div>
    <div className="space-y-3">
      {items.slice(0, 4).map((item, idx) => (
        <div
          key={idx}
          className="flex justify-between items-center text-lg font-mono bg-white/5 p-3 rounded border border-white/5"
        >
  ...
  return (
    <NodeContainer className={`${widthClass} border-acid-green/30`} delay={0.4}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="font-mono text-sm font-bold text-acid-green">
          AI AGENT
        </h3>
      </div>
      {!compact && (
        <div className="text-sm text-gray-300 font-mono mb-3">
          Analyzing:{" "}
          <span className="text-white block truncate">{taskName}</span>
        </div>
      )}
...
export const ScoringNode = ({ taskScore, skillScore, finalScore }) => (
  <NodeContainer className="w-[450px] border-white/50 text-center" delay={0.6}>
    ...
    <h3 className="font-mono text-xl text-gray-400 mb-4">
      REPLACABILITY SCORE
    </h3>
...
export const ScenarioNode = ({ title, description, likelihood }) => (
  <NodeContainer className="w-[500px] border-neon-cyan/40" delay={0.8}>
    <div className="flex items-center gap-4 mb-3">
      ...
      <h3 className="font-mono text-xl font-bold text-neon-cyan">
        FUTURE SCENARIO
      </h3>
    </div>
